...
const float GripperSpeed = 0.15;    // Gripper Speed in Degrees per Iteration
const float MaxGripDeg = 160;      // Maximum Gripper Servo Angle in Degrees
const float MinGripDeg = 20;       // Minimum Gripper Servo Angle in Degrees
...
void run_Scooper(int Triangle) {
  ...
  if ( !ScooperReady && (curMillisScoop - prevMillisScoop >= ScooperWaitTime))  {
    ...
    ScooperReady = true;
//    ScooperStartTime = millis(); // Start time when moving downwards
//    ScooperMoving = true;
//  }
//
//  // While scooper is moving, this branch runs
//  if (ScooperMoving) {
...
void run_Gripper(int L1, int R1, int Triangle) {
  if (Triangle) {
    GripperPos = 90;
    setPos(3, 90);
    setPos(4, 90);
