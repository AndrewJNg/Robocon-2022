...
// Function prototypes
void dispSpeed(int shootingSpeed) ;

// PS3 Status Variables
int player = 0;
int battery = 0;

// Shooting motor status
int startMotor = 0;
...
// SHOOTING MOTOR (IMPORTANT)
  if (Ps3.event.button_down.start ) startMotor = 1;
  if( Ps3.event.button_down.select ) startMotor = 0;
  
  // SHOOTING SPEED (IMPORTANT)
  if( Ps3.event.button_down.circle ) {
    if (shootingSpeed + 10 <= 255) {
      shootingSpeed += 10;
      Circle = 1;
    }
    dispSpeed(shootingSpeed);
  }
  if( Ps3.event.button_up.circle )      Circle = 0;
  
  if( Ps3.event.button_down.square ) {
    if (shootingSpeed - 10 >= 0) {
      shootingSpeed -= 10;
      Square = 1;
    }
    dispSpeed(shootingSpeed);
...
void dispSpeed(int shootingSpeed) {
    if (shootingSpeed >= 0 && shootingSpeed < 5) {
      Ps3.setPlayer(1);
    }
    else if (shootingSpeed >= 5 && shootingSpeed < 35) {
      Ps3.setPlayer(2);
    }
    else if (shootingSpeed >= 35 && shootingSpeed < 65) {
      Ps3.setPlayer(3);
    }
    else if (shootingSpeed >= 65 && shootingSpeed < 95) {
      Ps3.setPlayer(4);
    }
    else if (shootingSpeed >= 95 && shootingSpeed < 125) {
      Ps3.setPlayer(5);
    }
    else if (shootingSpeed >= 125 && shootingSpeed < 155) {
      Ps3.setPlayer(6);
    }
    else if (shootingSpeed >= 155 && shootingSpeed < 185) {
      Ps3.setPlayer(7);
    }
    else if (shootingSpeed >= 185 && shootingSpeed < 215) {
      Ps3.setPlayer(8);
    }
    else if (shootingSpeed >= 215 && shootingSpeed <= 245) {
      Ps3.setPlayer(9);
    }
    else if (shootingSpeed == 255) {
      Ps3.setPlayer(10);
    }
    else {
      Ps3.setPlayer(0);
